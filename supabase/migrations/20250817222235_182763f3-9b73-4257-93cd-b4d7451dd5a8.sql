...
WITH CHECK (
  ...
  user_type IS NOT DISTINCT FROM (SELECT user_type FROM public.profiles WHERE user_id = auth.uid()) AND
  staff_unit IS NOT DISTINCT FROM (SELECT staff_unit FROM public.profiles WHERE user_id = auth.uid()) AND
  staff_position IS NOT DISTINCT FROM (SELECT staff_position FROM public.profiles WHERE user_id = auth.uid())
);
