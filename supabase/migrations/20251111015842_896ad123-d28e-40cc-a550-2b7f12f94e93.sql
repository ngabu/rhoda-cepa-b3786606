-- Update calculate_application_fee to use truncation for accurate fee calculation
...
BEGIN
  ...
  -- Truncate daily rate to 3 decimal places (not round)
  v_daily_rate := TRUNC(v_annual_fee / 365.0, 3);

  -- Calculate total fee and round to 2 decimal places
  v_total_fee := ROUND(v_daily_rate * v_processing_days, 2);
