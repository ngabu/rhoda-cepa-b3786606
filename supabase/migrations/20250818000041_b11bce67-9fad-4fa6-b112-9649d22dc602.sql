-- First, create the missing compliance assessment for the application that should have triggered it
INSERT INTO public.compliance_assessments (
  ...
  'e1d33cae-fd61-4cdb-b903-280986d2487c',
  '00000000-0000-0000-0000-000000000000',  -- Placeholder, will be updated when assigned
  '0e0f48a9-6fcd-4fb2-9056-4192e923dd4f',  -- Registry officer who completed the assessment
  'pending',
  'Forwarded from registry - All validation satisfactory'
);

-- Now let's recreate the trigger to ensure it works properly
...
BEGIN
  ...
  IF NEW.assessment_status = 'passed' 
     ...
     AND (OLD.assessment_status != 'passed' 
          OR OLD.assessment_outcome != 'Approved for Next Stage') THEN
    ...
    -- Only create compliance assessment if one doesn't already exist
    IF NOT EXISTS (
