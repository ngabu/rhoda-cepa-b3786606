...
export function useRegistryTasks() {
  ...
  const fetchTasks = async () => {
    try {
      ...
      setTasks(tasksWithAssignees as RegistryTask[]);
    } catch (error) {
      console.error('Error fetching registry tasks:', error);
  ...
  const createTask = async (taskData: Omit<RegistryTask, 'id' | 'created_at' | 'updated_at' | 'assignee'>) => {
    try {
      const { data, error } = await supabase
        .from('registry_tasks')
        .insert([{
  ...
  const updateTask = async (taskId: string, updates: Partial<RegistryTask>) => {
    try {
