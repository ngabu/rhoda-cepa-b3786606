...
export function RevenuePermitsList() {
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, `Permits_Revenue_${format(new Date(), 'yyyy-MM-dd')}.xlsx`);
  };
  ...
  const exportToCSV = () => {
    ...
    link.download = `Permits_Revenue_${format(new Date(), 'yyyy-MM-dd')}.csv`;
    link.click();
  };
