...
export function ManagingDirectorEntitiesList() {
  ...
  const filteredEntities = useMemo(() => {
    return entities.filter((entity) => {
      ...
      const matchesProvince = provinceFilter === 'all' || entity.province === provinceFilter;
      
      return matchesSearch && matchesType && matchesStatus && matchesProvince;
    });
  }, [entities, searchTerm, entityTypeFilter, statusFilter, provinceFilter]);
  ...
  useMemo(() => {
    setCurrentPage(1);
  }, [searchTerm, entityTypeFilter, statusFilter, provinceFilter]);
  ...
  const provinces = useMemo(() => {
    return Array.from(new Set(entities.map(e => e.province).filter(Boolean)));
  }, [entities]);
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, `MD_Entities_${format(new Date(), 'yyyy-MM-dd')}.xlsx`);
  };
  ...
  const exportToCSV = () => {
    ...
    link.download = `MD_Entities_${format(new Date(), 'yyyy-MM-dd')}.csv`;
    link.click();
  };
            ...
            {filteredEntities.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="text-center text-muted-foreground">
                  {searchTerm || entityTypeFilter !== 'all' || statusFilter !== 'all' || provinceFilter !== 'all'
                ...
                return (
                  <>
                    ...
                    >
                      ...
                      <TableCell>{entity.province || '-'}</TableCell>
